...
test('returns truthful five-state Agent discovery for the opened project', async (t) => {
  ...
  assert.deepEqual(new Set(body.agents.map((agent) => agent.id)), new Set(['codex', 'claude-code', 'kimi-code', 'antigravity', ...(body.agents.some((agent) => agent.id === 'codebuddy') ? ['codebuddy'] : []), ...(body.agents.some((agent) => agent.id === 'qoder') ? ['qoder'] : []), ...(body.agents.some((agent) => agent.id === 'zcode') ? ['zcode'] : [])]));
