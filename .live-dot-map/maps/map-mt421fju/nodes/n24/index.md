...
### 1. 自动标注的显示效果演示
- **人类笔迹（上方段落）**：左侧为**琥珀金发丝细线**（Yellow/Amber）。
- **Agent 笔迹（当前段落）**：左侧为**靛蓝微紫发丝细线**（Indigo/Purple）。
- **切换到「预览」模式**：Agent 撰写的内容会自动封装在独立卡片内，顶部附带 `🤖 agent:antigravity` 专属徽标；而人类段落附带 `👤 人类` 徽标。两者视觉对比鲜明，优雅且不刺眼。

### 2. 关于“为什么前面文字会自动变黄”的原因与解答
- 在活点地图的 Markdown 规范中，作者标签是**流式向下继承**的：从某一行标注了 `<!-- @author: human -->` 开始，直到遇到下一个作者标签之前，后续所有段落都会被默认识别为该作者的笔迹。
- 之前由于文档上方标记了人类，中间没有终止或新的标签，所以只要在下方打字，系统就会判定为该段依然属于人类，从而显示黄线。
- **解决方案**：只要在需要区分的地方加入 `<!-- @author: agent:antigravity -->`（或在右下角微组件「笔迹 ▾」中点击切换），从该行往下的内容就会立即切为 Agent 的蓝紫发丝线，再也不会全部染黄了！
...
- **消除歧义操作**：底部的标记/取消操作已收拢到右下角极简的微型「●● 笔迹 ▾」色板中；
