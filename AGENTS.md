...
**当前阶段**：HTML 原型经 11 轮走查已定稿为验收样板（`canvas.html`，不再修改）；当前进入阶段 2——基于 `D:/桌面/活点地图/excalidraw` 的 fork 做画布可行性验证（7 项，见 `goal.md` 阶段 2），Excalidraw 源码已跑通、尚未改动。
...
├── 产品需求文档-PRD.md   # PRD v2.1：产品定义、数据模型、状态与连接规则、开发优先级
