...
安装器会先检测当前机器与项目中实际可用的 Agent，再只为检测到的 Agent 定向写入 MCP、hooks 和 Skill 配置；不要求用户先选择平台。已有配置会备份并合并，首次信任仍由对应 Agent 明确确认，安装器不能伪造“已连接”。缺少时初始化 map.json v2；已有 `map.json` 绝不覆盖。`doctor.ok` 必须为 `true`。

首次地图不会静默扫描项目。用户需要明确向已连接的 Agent 发送初始化请求（可直接复制）：
...
这条请求是一次性授权：Agent 只有收到它（或等价明确请求）后才按用户授权范围读取项目资料并写入首张地图；没有这条授权时只读地图，不扫描项目文件；已有地图永远不覆盖。
