...
> 数据格式详见 `map-json-v1.md`（v1.1，新增 `score`/`archived`/投影读取）；本文件只规定行为规则。
...
- 协议版本与 `map-json-v1.md` 同步演进；改 schema 必须回看本文件规则是否仍成立。v1.1：新增投影读取、整理例程、创新循环三节，语义规则补 `score`/`archived`。
