...
- `POST /api/v1/markdown/reveal` 只打开已校验路径；不执行用户提供的命令。GET 版本只
  返回存在性元数据，不产生打开副作用。
- **人机身份与成对闭合 Agent 块契约（方案 C）**：
  - Markdown 默认正文均为干净、纯粹的人类书写基线（不添加任何 `<!-- @author: human -->` 标签）。
  - Agent 生成或追加内容时，必须使用成对闭合标签包围整个回复：
    `<!-- @author: agent:<agentId> -->\n...正文...\n<!-- /@author -->`。
  - 严禁向文件末尾或自然段落遗留未闭合的 Agent 标签，以确保人类在闭合块外输入时 100% 保持纯净正文，绝不受标签污染。
- **高权重重点标记（`<mark>...</mark>`）**：
  - 人类可在编辑器中划选任意文字（无论人类自然文本还是 Agent 内容），设为「高权重重点」，文件内固化为标准 `<mark>…</mark>`。
  - 视觉上在编辑区与预览区均呈现克制高雅的暖金发丝点虚线与微光底色。
  - **Agent 语义感知与优先级**：后续 Agent 检索与理解上下文时，必须将 `<mark>` 包裹的内容识别为**人类亲自标注的高权重关键决策、核心意图与最高约束**，优先采纳并引用。Agent 追加生成内容时默认不得自行滥用 `<mark>`，且必须严格保留人类既有的 `<mark>` 标记，不得擅自抹除。
