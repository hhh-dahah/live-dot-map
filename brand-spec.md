...
:root {
  ...
  /* 警示黄 — 仅用于同步状态灯「待保存/未同步」,不作他用 */
  --warn:          oklch(72% 0.14 75);     /* 警示黄 ≈ #C9A227 */
  ...
  /* 人写标记候选色 — 仅用于编辑器 <mark> 人写段显示层(--note/--accent-soft 之外的第三选择) */
  --hl-blue:       oklch(93% 0.05 250);    /* 淡蓝高亮 */
}
```
...
- `--hl-blue` 仅用于编辑器内 `<mark>` 人写段的显示层高亮（与 --note/--accent-soft 同为可选标记色），不进文件、不用于其它元素。
- 编辑器/详情 dock 质感对标 codex：1px 发丝边框、小图标幽灵按钮（无边框、hover 出 `--surface-hover` 底色）、无大色块；唯一实心色块是主操作「保存」钮（--accent）。
