...
var ProjectStore = class _ProjectStore {
  ...
  constructor({
    ...
    this.dataDirectory = (0, import_node_path2.join)(projectRoot, MAP_DIRECTORY);
    this.mapPath = (0, import_node_path2.join)(this.dataDirectory, "map.json");
    this.bridgeDirectory = (0, import_node_path2.join)(this.dataDirectory, BRIDGE_DIRECTORY);
    this.walPath = (0, import_node_path2.join)(this.bridgeDirectory, "wal.ndjson");
    this.lockPath = (0, import_node_path2.join)(this.bridgeDirectory, "write.lock");
    this.snapshotDirectory = (0, import_node_path2.join)(this.bridgeDirectory, "snapshots");
    this.backupDirectory = (0, import_node_path2.join)(this.bridgeDirectory, "backups");
    this.quarantineDirectory = (0, import_node_path2.join)(this.bridgeDirectory, "quarantine");
  }
  ...
  async #pollExternal() {
    let metadata;
    try {
      metadata = await (0, import_promises2.stat)(this.mapPath);
    } catch {
      return;
    }
  ...
  async #captureDiskSignature() {
    try {
      const metadata = await (0, import_promises2.stat)(this.mapPath);
  ...
  async #assertSafeStoragePaths() {
    const canonicalRoot = await (0, import_promises2.realpath)(this.projectRoot);
    ...
    const rejectSymlink = async (path) => {
      try {
        if ((await (0, import_promises2.lstat)(path)).isSymbolicLink()) {
    ...
    for (const path of directories) {
      ...
      const escaped = (0, import_node_path3.relative)(canonicalRoot, canonical);
      if (escaped.startsWith("..") || (0, import_node_path3.isAbsolute)(escaped) || (0, import_node_path3.resolve)(canonical) === (0, import_node_path3.resolve)(canonicalRoot)) {
  ...
  async #initializeLocked() {
    ...
    if (!await exists(this.mapPath)) {
      const created = await this.shared.createEmptyMap({
        name: (0, import_node_path2.basename)(this.projectRoot),
  ...
  async #readWal() {
    ...
    const metadata = await (0, import_promises2.stat)(this.walPath);
    ...
    const content = await (0, import_promises2.readFile)(this.walPath, "utf8");
...
` : "";
  ...
  async #writeSnapshot(reason) {
    ...
    const path = (0, import_node_path2.join)(this.snapshotDirectory, `rev-${String(this.revision).padStart(12, "0")}-${timestampName(this.clock())}.json`);
...
`;
  ...
  async #pruneJsonDirectory(directory, keep) {
    const entries = (await (0, import_promises2.readdir)(directory, { withFileTypes: true })).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => entry.name).sort();
    await Promise.all(entries.slice(0, Math.max(0, entries.length - keep)).map((name) => (0, import_promises2.unlink)((0, import_node_path2.join)(directory, name))));
  }
  async #ensureDailyBackup() {
    const day = this.clock().toISOString().slice(0, 10);
    const path = (0, import_node_path2.join)(this.backupDirectory, `${day}.json`);
  ...
  async recover({ source = "snapshot", name } = {}) {
    return this.#exclusive(async () => {
      ...
      const entries = (await (0, import_promises2.readdir)(directory, { withFileTypes: true })).filter((entry) => entry.isFile() && entry.name.endsWith(".json")).map((entry) => entry.name).sort();
      const selected = name === void 0 ? entries.at(-1) : (0, import_node_path2.basename)(name);
      ...
      const envelope2 = await readJson((0, import_node_path2.join)(directory, selected));
...
function projectIdForRoot(projectRoot) {
  const digest = (0, import_node_crypto3.createHash)("sha256").update(String(projectRoot)).digest("hex").slice(0, 32);
...
function windowsDesktopDirectory({ platform = process.platform, env = process.env, exec = import_node_child_process.execFileSync } = {}) {
  if (platform !== "win32") return (0, import_node_path4.join)((0, import_node_os.homedir)(), "Desktop");
  try {
  ...
  return (0, import_node_path4.join)(env.USERPROFILE || (0, import_node_os.homedir)(), "Desktop");
}
async function createShortcut({
  ...
  env = process.env,
  exec = import_node_child_process.execFileSync
} = {}) {
  ...
  await (0, import_promises3.mkdir)(desktop, { recursive: true });
  if (platform !== "win32") {
    const launcher = (0, import_node_path4.join)(desktop, `${name}.command`);
    const script2 = `#!/bin/sh
exec ${JSON.stringify((0, import_node_path4.resolve)(target))}${args ? ` ${args}` : ""}
`;
    await (0, import_promises3.writeFile)(launcher, script2, { encoding: "utf8" });
    return { ok: true, type: "command", path: launcher };
  }
  const shortcut = (0, import_node_path4.join)(desktop, `${name}.lnk`);
  const script = [
    "$ws=New-Object -ComObject WScript.Shell",
    `$sc=$ws.CreateShortcut(${psQuote(shortcut)})`,
    `$sc.TargetPath=${psQuote((0, import_node_path4.resolve)(target))}`,
    args ? `$sc.Arguments=${psQuote(args)}` : "",
    `$sc.WorkingDirectory=${psQuote((0, import_node_path4.dirname)((0, import_node_path4.resolve)(target)))}`,
    "$sc.Save()"
  ].filter(Boolean).join(";");
  try {
    ...
    const fallback = (0, import_node_path4.join)(desktop, `${name}.cmd`);
    await (0, import_promises3.writeFile)(fallback, `@echo off
"${(0, import_node_path4.resolve)(target)}" ${args}
`, { encoding: "utf8" });
...
async function exists2(path) {
  try {
    await (0, import_promises4.access)(path, import_node_fs.constants.F_OK);
    return true;
  } catch {
    return false;
  }
...
async function atomicText(path, text) {
  ...
  const temp = `${path}.tmp-${process.pid}-${(0, import_node_crypto4.randomUUID)()}`;
  await (0, import_promises4.writeFile)(temp, text, { encoding: "utf8", flag: "wx" });
  await (0, import_promises4.rename)(temp, path);
}
...
async function readJson2(path, fallback = {}) {
  try {
    const value = JSON.parse(await (0, import_promises4.readFile)(path, "utf8"));
...
async function captureFile(path) {
  try {
    const metadata = await (0, import_promises4.stat)(path);
...
async function restoreCapturedFile(entry) {
  ...
  if (entry?.exists) {
    await (0, import_promises4.mkdir)((0, import_node_path5.dirname)(entry.path), { recursive: true });
    await (0, import_promises4.writeFile)(entry.path, Buffer.from(String(entry.content || ""), "base64"));
  } else {
    await (0, import_promises4.rm)(entry.path, { force: true }).catch(() => void 0);
  }
}
var adapterConfigPaths = (root, id) => id === "codex" ? [(0, import_node_path5.join)(root, ".codex", "config.toml"), (0, import_node_path5.join)(root, ".codex", "hooks.json")] : id === "claude-code" ? [(0, import_node_path5.join)(root, ".mcp.json"), (0, import_node_path5.join)(root, ".claude", "settings.json")] : id === "kimi-code" ? [(0, import_node_path5.join)(root, ".kimi-code", "mcp.json"), (0, import_node_path5.join)(root, ".live-dot-map", "kimi-plugin", "kimi.plugin.json"), (0, import_node_path5.join)(root, ".live-dot-map", "kimi-plugin", "runtime", "livedot.mjs")] : [(0, import_node_path5.join)(root, ".mcp.json"), (0, import_node_path5.join)(root, ".codebuddy", "settings.json"), (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin", ".codebuddy-plugin", "plugin.json"), (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin", ".workbuddy-plugin", "plugin.json")];
function seaRuntime() {
...
function execProbe(file, args = []) {
  return new Promise((resolve5) => {
    (0, import_node_child_process2.execFile)(file, args, { windowsHide: true, timeout: 4e3 }, (error, stdout = "") => {
...
function execText(file, args = [], timeout = 2500) {
  return new Promise((resolve5) => {
    (0, import_node_child_process2.execFile)(file, args, { windowsHide: true, timeout, encoding: "utf8" }, (error, stdout = "") => {
      resolve5(error ? "" : String(stdout));
    });
  });
}
...
async function discoverEmbeddedCodeBuddy({ platform = process.platform } = {}) {
  ...
  for (const iconPath of iconPaths) {
    const installRoot = (0, import_node_path5.dirname)(iconPath);
    const candidate = (0, import_node_path5.join)(installRoot, "resources", "app.asar.unpacked", "cli", "bin", "codebuddy");
...
async function detectInstalledAdapters({ projectRoot = process.cwd(), platform = process.platform } = {}) {
  ...
  const checks = await Promise.all(ALL_ADAPTERS.map(async (id) => {
    const configPaths = id === "codex" ? [(0, import_node_path5.join)(root, ".codex", "config.toml"), (0, import_node_path5.join)(root, ".codex", "hooks.json")] : id === "claude-code" ? [(0, import_node_path5.join)(root, ".mcp.json"), (0, import_node_path5.join)(root, ".claude", "settings.json")] : id === "kimi-code" ? [(0, import_node_path5.join)(root, ".kimi-code", "mcp.json"), (0, import_node_path5.join)(root, ".live-dot-map", "kimi-plugin", "kimi.plugin.json")] : [(0, import_node_path5.join)(root, ".codebuddy", "settings.json"), (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin", ".codebuddy-plugin", "plugin.json"), (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin", ".workbuddy-plugin", "plugin.json")];
    const configured = (await Promise.all(configPaths.map(async (path) => {
      const text = await (0, import_promises4.readFile)(path, "utf8").catch(() => "");
...
async function writeCodexConfig(root, nodeCommand, runtime) {
  const path = (0, import_node_path5.join)(root, ".codex", "config.toml");
  const begin = "# BEGIN LIVE-DOT-MAP";
  const end = "# END LIVE-DOT-MAP";
  const old = await (0, import_promises4.readFile)(path, "utf8").catch(() => "");
...
` : ""}${block}
`);
  await atomicJson((0, import_node_path5.join)(root, ".codex", "hooks.json"), mergeHooks(await readJson2((0, import_node_path5.join)(root, ".codex", "hooks.json")), hooksFor(nodeCommand, runtime, root, "codex")));
  return [path, (0, import_node_path5.join)(root, ".codex", "hooks.json")];
}
async function writeClaudeConfig(root, nodeCommand, runtime) {
  const settingsPath = (0, import_node_path5.join)(root, ".claude", "settings.json");
  await atomicJson(settingsPath, mergeHooks(await readJson2(settingsPath), hooksFor(nodeCommand, runtime, root, "claude")));
  const mcpPath = (0, import_node_path5.join)(root, ".mcp.json");
...
async function writeKimiConfig(root, nodeCommand, runtime) {
  const mcpPath = (0, import_node_path5.join)(root, ".kimi-code", "mcp.json");
  const mcp = await readJson2(mcpPath);
  mcp.mcpServers = { ...mcp.mcpServers || {}, "livedot-map": { command: nodeCommand, args: [...runtimeArgs(runtime), "mcp", "--project", root, "--agent", "kimi"] } };
  await atomicJson(mcpPath, mcp);
  const plugin = (0, import_node_path5.join)(root, ".live-dot-map", "kimi-plugin");
  const pluginRuntime = (0, import_node_path5.join)(plugin, "runtime", "livedot.mjs");
  await (0, import_promises4.mkdir)((0, import_node_path5.dirname)(pluginRuntime), { recursive: true });
  if (!seaRuntime()) await (0, import_promises4.copyFile)(runtime, pluginRuntime);
  ...
  await atomicJson((0, import_node_path5.join)(plugin, "kimi.plugin.json"), manifest);
  return [mcpPath, (0, import_node_path5.join)(plugin, "kimi.plugin.json")];
}
async function writeCodeBuddyConfig(root, nodeCommand, runtime) {
  const settingsPath = (0, import_node_path5.join)(root, ".codebuddy", "settings.json");
  ...
  await atomicJson(mcpPath, mcp);
  const plugin = (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin");
  const manifest = {
    name: "livedot-map",
    version: "2.0.0",
  ...
  await atomicJson((0, import_node_path5.join)(plugin, ".codebuddy-plugin", "plugin.json"), manifest);
  await atomicJson((0, import_node_path5.join)(plugin, ".workbuddy-plugin", "plugin.json"), manifest);
  await atomicJson((0, import_node_path5.join)(plugin, "hooks", "hooks.json"), { hooks: hooksFor(nodeCommand, runtime, root, "codebuddy") });
  return [settingsPath, mcpPath, (0, import_node_path5.join)(plugin, ".codebuddy-plugin", "plugin.json"), (0, import_node_path5.join)(plugin, ".workbuddy-plugin", "plugin.json"), (0, import_node_path5.join)(plugin, "hooks", "hooks.json")];
}
async function installProject({
  ...
  const source = (0, import_node_path5.resolve)(sourceRoot instanceof URL ? (0, import_node_url.fileURLToPath)(sourceRoot) : sourceRoot || process.cwd());
  const sourceRuntime = (0, import_node_path5.resolve)(runtimeSource || (0, import_node_path5.resolve)(source, "livedot.mjs"));
  if (!seaRuntime() && !await exists2(sourceRuntime)) throw new Error(`\u7F3A\u5C11\u5DF2\u6784\u5EFA\u8FD0\u884C\u65F6: ${sourceRuntime}`);
  const dataDir = (0, import_node_path5.join)(root, ".live-dot-map");
  const runtime = seaRuntime() ? null : (0, import_node_path5.join)(dataDir, "livedot.mjs");
  await (0, import_promises4.mkdir)(dataDir, { recursive: true });
  const projectId = projectIdForRoot(root);
  const mapPath = (0, import_node_path5.join)(dataDir, "map.json");
  const configPath = (0, import_node_path5.join)(dataDir, "agent-kit.json");
  ...
  const backupPath = (0, import_node_path5.join)(dataDir, "backups", "agent-kit-install.json");
  ...
  const rollback = async () => {
    ...
    if (!oldMap.exists) await (0, import_promises4.rm)(mapPath, { force: true }).catch(() => void 0);
    if (runtime && !oldRuntime.exists) await (0, import_promises4.rm)(runtime, { force: true }).catch(() => void 0);
  };
  try {
    await atomicJson(backupPath, backup);
    if (runtime && (0, import_node_path5.resolve)(sourceRuntime) !== (0, import_node_path5.resolve)(runtime)) await (0, import_promises4.copyFile)(sourceRuntime, runtime);
    if (!oldMap.exists) {
      const map = structuredClone(map_template_default);
      if (map.version !== 2) throw new Error("\u5185\u7F6E map.json \u6A21\u677F\u4E0D\u662F v2");
      const now = (/* @__PURE__ */ new Date()).toISOString();
      map.mapId = projectId;
      map.name = (0, import_node_path5.basename)(root);
      map.createdAt = now;
      map.updatedAt = now;
    ...
    const result = {
      ...
      shortcut: null,
      trustRequired: Object.fromEntries(Object.keys(installed).map((id) => [id, id === "codex" ? "\u5728 Codex /hooks \u4E2D\u4FE1\u4EFB\u9879\u76EE hooks" : id === "claude-code" ? "\u9996\u6B21\u6253\u5F00\u9879\u76EE\u65F6\u786E\u8BA4 hooks \u4E0E MCP" : id === "kimi-code" ? `\u5728 Kimi \u6267\u884C /plugins install ${(0, import_node_path5.join)(dataDir, "kimi-plugin")}` : "\u5728 WorkBuddy/CodeBuddy \u63D2\u4EF6\u9762\u677F\u5BA1\u6838\u5E76\u542F\u7528 hooks \u4E0E MCP"])),
    ...
    if (createDesktopShortcut && platform === "win32") {
      const launcher = (0, import_node_path5.join)(dataDir, "\u542F\u52A8\u6D3B\u70B9\u5730\u56FE.cmd");
      const app = (0, import_node_path5.resolve)(appPath || (0, import_node_path5.join)(root, "app.html"));
...
`);
      try {
        ...
        const fallback = (0, import_node_path5.join)(dataDir, "\u6253\u5F00\u6D3B\u70B9\u5730\u56FE.cmd");
...
async function uninstallProject({ projectRoot = process.cwd(), platform = process.platform, env = process.env, exec } = {}) {
  ...
  if (!config || typeof config !== "object") return { ok: false, reason: "not-installed", projectRoot: root, mapPreserved: await exists2((0, import_node_path5.join)(dataDir, "map.json")) };
  const backupPath = typeof config.installBackup === "string" ? config.installBackup : (0, import_node_path5.join)(dataDir, "backups", "agent-kit-install.json");
  ...
  for (const entry of Array.isArray(backup?.files) ? backup.files : []) {
    if (!entry?.path || entry.path === (0, import_node_path5.join)(dataDir, "map.json") || entry.path === backupPath) continue;
  ...
  const launcherPaths = [(0, import_node_path5.join)(dataDir, "\u542F\u52A8\u6D3B\u70B9\u5730\u56FE.cmd"), (0, import_node_path5.join)(dataDir, "\u6253\u5F00\u6D3B\u70B9\u5730\u56FE.cmd")];
  if (platform === "win32") {
    const desktop = windowsDesktopDirectory({ platform, env, exec });
    launcherPaths.push((0, import_node_path5.join)(desktop, "\u6D3B\u70B9\u5730\u56FE\u672C\u5730\u6865.lnk"), (0, import_node_path5.join)(desktop, "\u6D3B\u70B9\u5730\u56FE\u672C\u5730\u6865.cmd"));
  }
  ...
  const mapPreserved = await exists2((0, import_node_path5.join)(dataDir, "map.json"));
...
async function doctorProject({ projectRoot = process.cwd(), checkBridge = false, bridgeClient, offline = true } = {}) {
  const root = (0, import_node_path5.resolve)(projectRoot);
  const configPath = (0, import_node_path5.join)(root, ".live-dot-map", "agent-kit.json");
  const config = await readJson2(configPath, null);
  const installed = config?.installed && typeof config.installed === "object" ? config.installed : {};
  const expected = [
    ["agent-kit-config", configPath],
    ["map", (0, import_node_path5.join)(root, ".live-dot-map", "map.json")]
  ];
  if (config?.runtimeMode !== "sea" && config?.runtime !== null) expected.push(["runtime", (0, import_node_path5.join)(root, ".live-dot-map", "livedot.mjs")]);
  if (installed.codex) expected.push(["codex-hooks", (0, import_node_path5.join)(root, ".codex", "hooks.json")], ["codex-mcp", (0, import_node_path5.join)(root, ".codex", "config.toml")]);
  if (installed["claude-code"]) expected.push(["claude-hooks", (0, import_node_path5.join)(root, ".claude", "settings.json")], ["claude-mcp", (0, import_node_path5.join)(root, ".mcp.json")]);
  if (installed["kimi-code"]) expected.push(["kimi-mcp", (0, import_node_path5.join)(root, ".kimi-code", "mcp.json")], ["kimi-plugin", (0, import_node_path5.join)(root, ".live-dot-map", "kimi-plugin", "kimi.plugin.json")]);
  if (installed.codebuddy) expected.push(["codebuddy-hooks", (0, import_node_path5.join)(root, ".codebuddy", "settings.json")], ["codebuddy-mcp", (0, import_node_path5.join)(root, ".mcp.json")], ["codebuddy-plugin", (0, import_node_path5.join)(root, ".live-dot-map", "codebuddy-plugin", ".codebuddy-plugin", "plugin.json")]);
...
function randomToken(bytes = 32) {
  return (0, import_node_crypto5.randomBytes)(bytes).toString("base64url");
}
  ...
  const server = (0, import_node_http.createServer)(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/agents") {
        ...
        let config = {};
        try {
          const parsed = JSON.parse(await (0, import_promises5.readFile)((0, import_node_path6.join)(root, ".live-dot-map", "agent-kit.json"), "utf8"));
...
async function markdownDocuments(root, limit = 200) {
  ...
  const walk = async (directory, depth) => {
    ...
    const entries = await (0, import_promises6.readdir)(directory, { withFileTypes: true }).catch(() => []);
    for (const entry of entries) {
      if (output.length >= limit || ignored.has(entry.name)) continue;
      const full = (0, import_node_path7.join)(directory, entry.name);
      if (entry.isDirectory()) await walk(full, depth + 1);
      else if (entry.isFile() && (0, import_node_path7.extname)(entry.name).toLowerCase() === ".md") {
        const metadata = await (0, import_promises6.stat)(full).catch(() => null);
        if (!metadata || metadata.size > 2e6) continue;
        const text = await (0, import_promises6.readFile)(full, "utf8").catch(() => "");
...
async function openStore(projectRoot) {
  return ProjectStore.open({ projectRoot: (0, import_node_path7.resolve)(projectRoot), shared: await loadSharedAdapter(), pollIntervalMs: 0 });
}
function envelope(projectId, revision, actor, sessionId, commands) {
  return { projectId, baseRevision: revision, commandId: `cmd-${(0, import_node_crypto6.randomUUID)()}`, actor, sessionId, commands };
}
async function callTool(store, root, tool, args, defaultActor = "agent:generic") {
  ...
  const sessionId = typeof args.sessionId === "string" ? args.sessionId : `session-${(0, import_node_crypto6.randomUUID)()}`;
  if (tool === "map_get_context") {
  ...
  if (tool === "map_apply_commands") {
    const request = args.envelope && typeof args.envelope === "object" ? args.envelope : {
      ...
      commandId: typeof args.commandId === "string" ? args.commandId : `cmd-${(0, import_node_crypto6.randomUUID)()}`
    };
...
async function runMcp(projectRoot, actor) {
  const root = (0, import_node_path7.resolve)(projectRoot);
...
async function runHook(kind, args) {
  const root = (0, import_node_path7.resolve)(required(args, "project"));
  const actor = `agent:${String(args.agent || "generic")}`;
  const sessionId = String(args.session || `session-${(0, import_node_crypto6.randomUUID)()}`);
  const store = await openStore(root);
  const snapshot = await store.snapshot();
  const document = snapshot.document;
...
async function main() {
  ...
  if (command2 === "serve") {
    const projectRoot = (0, import_node_path7.resolve)(required(args, "project"));
    const appPath = (0, import_node_path7.resolve)(typeof args.app === "string" ? args.app : (0, import_node_path7.join)(process.cwd(), "app.html"));
    const appHtml = await (0, import_promises6.readFile)(appPath, "utf8");
    const assetRoot = (0, import_node_path7.dirname)(appPath);
    const staticAssets = {};
    for (const [urlPath, file, type] of [
      ["/sw.js", "sw.js", "text/javascript; charset=utf-8"],
      ["/manifest.webmanifest", "manifest.webmanifest", "application/manifest+json; charset=utf-8"],
      ["/icons/icon-192.png", (0, import_node_path7.join)("icons", "icon-192.png"), "image/png"],
      ["/icons/icon-512.png", (0, import_node_path7.join)("icons", "icon-512.png"), "image/png"]
    ]) {
      try {
        staticAssets[urlPath] = { body: await (0, import_promises6.readFile)((0, import_node_path7.join)(assetRoot, file)), type };
      } catch {
      }
    }
...
`);
  ...
  if (command2 === "install") {
    const root = (0, import_node_path7.resolve)(typeof args.project === "string" ? args.project : process.cwd());
    const runtimeSource = process.env.LIVEDOT_RUNTIME_SOURCE || process.argv[1] || process.cwd();
    const appPath = (0, import_node_path7.resolve)(typeof args.app === "string" ? args.app : (0, import_node_path7.join)((0, import_node_path7.dirname)(runtimeSource), "app.html"));
