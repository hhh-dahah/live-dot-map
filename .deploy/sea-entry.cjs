...
async function withFileLock(path, operation, { timeoutMs = 5e3, staleMs = 3e4 } = {}) {
  ...
  try {
    ...
    await (0, import_promises.unlink)(path).catch((error) => {
...
async function listMapIds(projectRoot) {
  const entries = await (0, import_promises2.readdir)(mapsRoot(projectRoot), { withFileTypes: true }).catch((error) => {
...
async function rejectSymlink(path) {
  const metadata = await (0, import_promises2.lstat)(path).catch((error) => {
    if (error?.code === "ENOENT") return null;
    throw error;
  });
...
var MarkdownStore = class {
  ...
  async reveal(requestedPath, { open: open2 = false } = {}) {
    const { path, candidate } = await this.#target(requestedPath, { allowMissing: true });
    const exists3 = await (0, import_promises5.stat)(candidate).then(() => true).catch((error) => error?.code === "ENOENT" ? false : Promise.reject(error));
    let opened = false;
    if (open2) {
      const target = exists3 ? candidate : (0, import_node_path6.dirname)(candidate);
      try {
        const child = process.platform === "win32" ? (0, import_node_child_process.spawn)("explorer.exe", ["/select,", target], { detached: true, stdio: "ignore" }) : process.platform === "darwin" ? (0, import_node_child_process.spawn)("open", ["-R", target], { detached: true, stdio: "ignore" }) : (0, import_node_child_process.spawn)("xdg-open", [(0, import_node_path6.dirname)(target)], { detached: true, stdio: "ignore" });
        child.once("error", () => {
        });
        child.unref();
        opened = true;
...
function projectIdForRoot(projectRoot) {
  const digest2 = (0, import_node_crypto4.createHash)("sha256").update(String(projectRoot)).digest("hex").slice(0, 32);
...
function windowsDesktopDirectory({ platform = process.platform, env = process.env, exec = import_node_child_process2.execFileSync } = {}) {
  if (platform !== "win32") return (0, import_node_path7.join)((0, import_node_os2.homedir)(), "Desktop");
  try {
  ...
  return (0, import_node_path7.join)(env.USERPROFILE || (0, import_node_os2.homedir)(), "Desktop");
}
...
var skillTargetPaths = (home, id) => id === "codex" ? (0, import_node_path8.join)(home, ".codex", "skills", "live-dot-map", "SKILL.md") : id === "claude-code" ? (0, import_node_path8.join)(home, ".claude", "skills", "live-dot-map", "SKILL.md") : id === "kimi-code" ? (0, import_node_path8.join)(home, ".kimi-code", "plugins", "live-dot-map", "skills", "live-dot-map", "SKILL.md") : (0, import_node_path8.join)(home, ".codebuddy", "plugins", "live-dot-map", "skills", "live-dot-map", "SKILL.md");
var kimiPluginRoot = (home) => (0, import_node_path8.join)(home, ".kimi-code", "plugins", "live-dot-map");
var codebuddyPluginRoot = (home) => (0, import_node_path8.join)(home, ".codebuddy", "plugins", "live-dot-map");
...
async function exists2(path) {
  try {
    await (0, import_promises6.access)(path, import_node_fs.constants.F_OK);
    return true;
  } catch {
    return false;
  }
...
async function atomicText(path, text) {
  await (0, import_promises6.mkdir)((0, import_node_path8.dirname)(path), { recursive: true });
  const temp = `${path}.tmp-${process.pid}-${(0, import_node_crypto5.randomUUID)()}`;
  await (0, import_promises6.writeFile)(temp, text, { encoding: "utf8", flag: "wx" });
  await (0, import_promises6.rename)(temp, path);
}
...
async function readJson2(path, fallback = {}) {
  try {
    const value = JSON.parse(await (0, import_promises6.readFile)(path, "utf8"));
...
async function captureFile(path) {
  try {
    const metadata = await (0, import_promises6.stat)(path);
    if (metadata.isDirectory()) return { path, exists: true, kind: "directory", sha256: null, content: null };
    const bytes = await (0, import_promises6.readFile)(path);
...
async function restoreCapturedFile(entry) {
  ...
  if (entry?.exists) {
    await (0, import_promises6.mkdir)((0, import_node_path8.dirname)(entry.path), { recursive: true });
    await (0, import_promises6.writeFile)(entry.path, Buffer.from(String(entry.content || ""), "base64"));
  } else {
    await (0, import_promises6.rm)(entry.path, { force: true }).catch(() => void 0);
  }
}
var adapterConfigPaths = (home, id) => id === "codex" ? [(0, import_node_path8.join)(home, ".codex", "config.toml"), (0, import_node_path8.join)(home, ".codex", "hooks.json")] : id === "claude-code" ? [(0, import_node_path8.join)(home, ".claude", "settings.json")] : id === "kimi-code" ? [(0, import_node_path8.join)(home, ".kimi-code", "mcp.json"), (0, import_node_path8.join)(kimiPluginRoot(home), "kimi.plugin.json")] : [(0, import_node_path8.join)(home, ".codebuddy", "settings.json"), (0, import_node_path8.join)(codebuddyPluginRoot(home), ".codebuddy-plugin", "plugin.json"), (0, import_node_path8.join)(codebuddyPluginRoot(home), ".workbuddy-plugin", "plugin.json"), (0, import_node_path8.join)(codebuddyPluginRoot(home), "hooks", "hooks.json")];
function seaRuntime() {
...
function execProbe(file, args = []) {
  return new Promise((resolve7) => {
    (0, import_node_child_process3.execFile)(file, args, { windowsHide: true, timeout: 4e3 }, (error, stdout = "") => {
...
function execText(file, args = [], timeout = 2500) {
  return new Promise((resolve7) => {
    (0, import_node_child_process3.execFile)(file, args, { windowsHide: true, timeout, encoding: "utf8" }, (error, stdout = "") => {
...
async function discoverEmbeddedCodeBuddy({ platform = process.platform } = {}) {
  ...
  for (const iconPath of iconPaths) {
    const installRoot = (0, import_node_path8.dirname)(iconPath);
    const candidate = (0, import_node_path8.join)(installRoot, "resources", "app.asar.unpacked", "cli", "bin", "codebuddy");
    if (await exists2(candidate)) return candidate;
  }
  return null;
}
async function detectInstalledAdapters({ projectRoot = process.cwd(), platform = process.platform, homeRoot = (0, import_node_os3.homedir)() } = {}) {
  const root = (0, import_node_path8.resolve)(projectRoot);
  const home = (0, import_node_path8.resolve)(homeRoot);
  const checks = await Promise.all(ALL_ADAPTERS.map(async (id) => {
    ...
    const configured = (await Promise.all(configPaths.map(async (path) => {
      const text = await (0, import_promises6.readFile)(path, "utf8").catch(() => "");
...
async function writeCodexConfig(home, nodeCommand, runtime) {
  const path = (0, import_node_path8.join)(home, ".codex", "config.toml");
  const begin = "# BEGIN LIVE-DOT-MAP";
  const end = "# END LIVE-DOT-MAP";
  const old = await (0, import_promises6.readFile)(path, "utf8").catch(() => "");
...
` : ""}${block}
`);
  const hooksPath = (0, import_node_path8.join)(home, ".codex", "hooks.json");
...
async function writeClaudeConfig(home, nodeCommand, runtime) {
  const settingsPath = (0, import_node_path8.join)(home, ".claude", "settings.json");
...
async function writeKimiConfig(home, nodeCommand, runtime) {
  const mcpPath = (0, import_node_path8.join)(home, ".kimi-code", "mcp.json");
  ...
  const pluginRuntime = (0, import_node_path8.join)(plugin, "runtime", "livedot.mjs");
  await (0, import_promises6.mkdir)((0, import_node_path8.dirname)(pluginRuntime), { recursive: true });
  if (!seaRuntime()) await (0, import_promises6.copyFile)(runtime, pluginRuntime);
  const manifest = {
    name: "livedot-map",
    version: "2.0.0",
  ...
  await atomicJson((0, import_node_path8.join)(plugin, "kimi.plugin.json"), manifest);
  return [mcpPath, (0, import_node_path8.join)(plugin, "kimi.plugin.json")];
}
async function writeCodeBuddyConfig(home, nodeCommand, runtime) {
  const settingsPath = (0, import_node_path8.join)(home, ".codebuddy", "settings.json");
  const settings = await readJson2(settingsPath);
  const mcp = { mcpServers: settings.mcpServers && typeof settings.mcpServers === "object" ? settings.mcpServers : {} };
  ...
  await atomicJson((0, import_node_path8.join)(plugin, ".codebuddy-plugin", "plugin.json"), manifest);
  await atomicJson((0, import_node_path8.join)(plugin, ".workbuddy-plugin", "plugin.json"), manifest);
  await atomicJson((0, import_node_path8.join)(plugin, "hooks", "hooks.json"), { hooks: hooksFor(nodeCommand, runtime, "codebuddy") });
  return [settingsPath, (0, import_node_path8.join)(plugin, ".codebuddy-plugin", "plugin.json"), (0, import_node_path8.join)(plugin, ".workbuddy-plugin", "plugin.json"), (0, import_node_path8.join)(plugin, "hooks", "hooks.json")];
}
async function installProject({
  ...
  discoverAgents = true,
  detectedAgents = null,
  homeRoot = (0, import_node_os3.homedir)()
} = {}) {
  const root = (0, import_node_path8.resolve)(projectRoot);
  const home = (0, import_node_path8.resolve)(homeRoot);
  if (!await exists2(root)) throw new Error(`\u9879\u76EE\u76EE\u5F55\u4E0D\u5B58\u5728: ${root}`);
  const source = (0, import_node_path8.resolve)(sourceRoot instanceof URL ? (0, import_node_url.fileURLToPath)(sourceRoot) : sourceRoot || process.cwd());
  const sourceRuntime = (0, import_node_path8.resolve)(runtimeSource || (0, import_node_path8.resolve)(source, "livedot.mjs"));
  const canonicalCandidates = [(0, import_node_path8.resolve)(source, "skills", "live-dot-map", "SKILL.md"), (0, import_node_path8.resolve)(source, "agent-kit", "skills", "live-dot-map", "SKILL.md")];
  ...
  const dataDir = (0, import_node_path8.join)(root, ".live-dot-map");
  const globalDataDir = (0, import_node_path8.join)(home, ".live-dot-map");
  const runtime = seaRuntime() ? null : (0, import_node_path8.join)(globalDataDir, "livedot.mjs");
  await (0, import_promises6.mkdir)(dataDir, { recursive: true });
  const projectId = projectIdForRoot(root);
  const mapPath = (0, import_node_path8.join)(dataDir, "map.json");
  const configPath = (0, import_node_path8.join)(dataDir, "agent-kit.json");
  ...
  const backupPath = (0, import_node_path8.join)(globalDataDir, "backups", `agent-kit-install-${projectId.replace(/[^a-zA-Z0-9_-]/g, "_")}.json`);
  ...
  const mapsLayoutExists = await exists2((0, import_node_path8.join)(dataDir, "maps"));
  ...
  const rollback = async () => {
    ...
    if (!oldMap.exists) await (0, import_promises6.rm)(mapPath, { force: true }).catch(() => void 0);
    if (createdMapsLayout) {
      await (0, import_promises6.rm)((0, import_node_path8.join)(dataDir, "maps"), { recursive: true, force: true }).catch(() => void 0);
      await (0, import_promises6.rm)((0, import_node_path8.join)(dataDir, "active-map"), { force: true }).catch(() => void 0);
    }
    if (runtime && !oldRuntime.exists) await (0, import_promises6.rm)(runtime, { force: true }).catch(() => void 0);
  };
  try {
    await atomicJson(backupPath, backup);
    if (runtime && (0, import_node_path8.resolve)(sourceRuntime) !== (0, import_node_path8.resolve)(runtime)) {
      await (0, import_promises6.mkdir)(globalDataDir, { recursive: true });
      await (0, import_promises6.copyFile)(sourceRuntime, runtime);
    }
    for (const id of Object.keys(installed)) {
      const target = skillTargetPaths(home, id);
      await (0, import_promises6.mkdir)((0, import_node_path8.dirname)(target), { recursive: true });
      await (0, import_promises6.copyFile)(canonicalSkill, target);
    }
    if (!oldMap.exists && !mapsLayoutExists) {
      ...
      map.mapId = projectId;
      map.name = (0, import_node_path8.basename)(root);
      map.createdAt = now;
      map.updatedAt = now;
      ...
      await atomicJson((0, import_node_path8.join)(dataDir, "maps", "default", "map.json"), map);
      await atomicText((0, import_node_path8.join)(dataDir, "active-map"), "default\n");
...
async function uninstallProject({ projectRoot = process.cwd(), platform = process.platform, env = process.env, exec } = {}) {
  ...
  if (!config || typeof config !== "object") return { ok: false, reason: "not-installed", projectRoot: root, mapPreserved: await exists2((0, import_node_path8.join)(dataDir, "map.json")) || await exists2((0, import_node_path8.join)(dataDir, "maps")) };
  const backupPath = typeof config.installBackup === "string" ? config.installBackup : (0, import_node_path8.join)(dataDir, "backups", "agent-kit-install.json");
  ...
  for (const entry of Array.isArray(backup?.files) ? backup.files : []) {
    if (!entry?.path || entry.path === (0, import_node_path8.join)(dataDir, "map.json") || entry.path === backupPath) continue;
  ...
  const launcherPaths = [(0, import_node_path8.join)(dataDir, "\u542F\u52A8\u6D3B\u70B9\u5730\u56FE.cmd"), (0, import_node_path8.join)(dataDir, "\u6253\u5F00\u6D3B\u70B9\u5730\u56FE.cmd")];
  if (platform === "win32") {
    const desktop = windowsDesktopDirectory({ platform, env, exec });
    launcherPaths.push((0, import_node_path8.join)(desktop, "\u6D3B\u70B9\u5730\u56FE\u672C\u5730\u6865.lnk"), (0, import_node_path8.join)(desktop, "\u6D3B\u70B9\u5730\u56FE\u672C\u5730\u6865.cmd"));
  }
  ...
  const mapPreserved = await exists2((0, import_node_path8.join)(dataDir, "map.json")) || await exists2((0, import_node_path8.join)(dataDir, "maps"));
  return { ok: skipped.length === 0, projectRoot: root, restored, skipped, mapPreserved, backupPath };
}
async function doctorProject({ projectRoot = process.cwd(), checkBridge = false, bridgeClient, offline = true, homeRoot = (0, import_node_os3.homedir)() } = {}) {
  const root = (0, import_node_path8.resolve)(projectRoot);
  const home = (0, import_node_path8.resolve)(homeRoot);
  const configPath = (0, import_node_path8.join)(root, ".live-dot-map", "agent-kit.json");
  const config = await readJson2(configPath, null);
  const installed = config?.installed && typeof config.installed === "object" ? config.installed : {};
  const expected = [
    ["agent-kit-config", configPath]
  ];
  if (config?.runtimeMode !== "sea" && config?.runtime !== null) expected.push(["runtime", (0, import_node_path8.join)(home, ".live-dot-map", "livedot.mjs")]);
  if (installed.codex) expected.push(["codex-hooks", (0, import_node_path8.join)(home, ".codex", "hooks.json")], ["codex-mcp", (0, import_node_path8.join)(home, ".codex", "config.toml")]);
  if (installed["claude-code"]) expected.push(["claude-hooks", (0, import_node_path8.join)(home, ".claude", "settings.json")]);
  if (installed["kimi-code"]) expected.push(["kimi-mcp", (0, import_node_path8.join)(home, ".kimi-code", "mcp.json")], ["kimi-plugin", (0, import_node_path8.join)(kimiPluginRoot(home), "kimi.plugin.json")]);
  if (installed.codebuddy) expected.push(["codebuddy-hooks", (0, import_node_path8.join)(home, ".codebuddy", "settings.json")], ["codebuddy-plugin", (0, import_node_path8.join)(codebuddyPluginRoot(home), ".codebuddy-plugin", "plugin.json")]);
  ...
  checks.push({ name: "map", ok: await exists2((0, import_node_path8.join)(root, ".live-dot-map", "map.json")) || await exists2((0, import_node_path8.join)(root, ".live-dot-map", "maps")), detail: (0, import_node_path8.join)(root, ".live-dot-map") });
...
var RECENT_PROJECTS_FILE = () => process.env.LIVEDOT_RECENT_PROJECTS_FILE || (0, import_node_path9.join)((0, import_node_os4.homedir)(), ".live-dot-map", "recent-projects.json");
async function recordRecentProject(root) {
  let recent = [];
  try {
    const parsed = JSON.parse(await (0, import_promises7.readFile)(RECENT_PROJECTS_FILE(), "utf8"));
    if (Array.isArray(parsed)) recent = parsed.filter((item) => typeof item === "string");
  } catch {
  }
  recent = [root, ...recent.filter((item) => item !== root)].slice(0, 15);
  await (0, import_promises7.mkdir)((0, import_node_path9.dirname)(RECENT_PROJECTS_FILE()), { recursive: true });
  await (0, import_promises7.writeFile)(RECENT_PROJECTS_FILE(), `${JSON.stringify(recent, null, 2)}
`, "utf8");
}
...
async function pickProjectFolder({ logger = noopLogger } = {}) {
  const tmpDir = (0, import_node_path9.join)((0, import_node_os4.homedir)(), ".live-dot-map", "tmp");
  await (0, import_promises7.mkdir)(tmpDir, { recursive: true });
  const marker = (0, import_node_path9.join)(tmpDir, `pick-${(0, import_node_crypto6.randomUUID)()}.txt`);
  const script = buildPickFolderScript(marker);
  const run = await new Promise((resolveRun, rejectRun) => {
    const child = (0, import_node_child_process4.spawn)("powershell.exe", ["-NoProfile", "-STA", "-ExecutionPolicy", "Bypass", "-Command", script], { windowsHide: true });
    let stdout = "";
    let stderr = "";
  ...
  if (run.code !== 0) {
    ...
    await (0, import_promises7.rm)(marker, { force: true }).catch(() => void 0);
    return { cancelled: true };
  }
  try {
    const text = (await (0, import_promises7.readFile)(marker, "utf8")).trim();
    await (0, import_promises7.rm)(marker, { force: true }).catch(() => void 0);
    if (text) {
...
function randomToken(bytes = 32) {
  return (0, import_node_crypto6.randomBytes)(bytes).toString("base64url");
}
async function markdownDocuments(root, limit = 200) {
  ...
  const walk = async (directory, depth) => {
    ...
    const entries = await (0, import_promises7.readdir)(directory, { withFileTypes: true }).catch(() => []);
    for (const entry of entries) {
      if (output.length >= limit || ignored.has(entry.name)) continue;
      const full = (0, import_node_path9.join)(directory, entry.name);
      if (entry.isDirectory()) await walk(full, depth + 1);
      else if (entry.isFile() && (0, import_node_path9.extname)(entry.name).toLowerCase() === ".md") {
        const metadata = await (0, import_promises7.stat)(full).catch(() => null);
        if (!metadata || metadata.size > 2e6) continue;
        const text = await (0, import_promises7.readFile)(full, "utf8").catch(() => "");
...
async function recordAgentHealth(root, actor, event, status, error) {
  const path = (0, import_node_path9.join)(root, ".live-dot-map", ".bridge", "agent-health.json");
  const prior = await (0, import_promises7.readFile)(path, "utf8").then((text) => JSON.parse(text)).catch(() => ({}));
  ...
  await (0, import_promises7.mkdir)((0, import_node_path9.join)(root, ".live-dot-map", ".bridge"), { recursive: true });
  const temporary = `${path}.${process.pid}.${randomToken(8)}.tmp`;
  try {
    await (0, import_promises7.writeFile)(temporary, `${JSON.stringify({ version: 1, updatedAt: (/* @__PURE__ */ new Date()).toISOString(), records }, null, 2)}
`, "utf8");
...
async function readAgentHealth(root) {
  return (0, import_promises7.readFile)((0, import_node_path9.join)(root, ".live-dot-map", ".bridge", "agent-health.json"), "utf8").then((text) => {
...
function runtimeSources({ sourceRoot, runtimeSource } = {}) {
  const entry = process.argv[1] ? (0, import_node_path9.resolve)(process.argv[1]) : "";
  const entryRoot = entry ? (0, import_node_path9.dirname)(entry) : "";
  const roots = [
    sourceRoot,
    process.env.LIVEDOT_AGENT_KIT_SOURCE,
    process.cwd(),
    entryRoot
  ].filter(Boolean).map((value) => (0, import_node_path9.resolve)(value));
  const uniqueRoots = [...new Set(roots)];
  const runtimes = [
    runtimeSource,
    process.env.LIVEDOT_RUNTIME_SOURCE,
    ...uniqueRoots.map((root) => (0, import_node_path9.join)(root, "livedot.mjs"))
  ].filter(Boolean).map((value) => (0, import_node_path9.resolve)(value));
...
async function ensureProjectAgentConfig(projectRoot, {
  ...
  try {
    ...
    const configPath = (0, import_node_path9.join)(root, ".live-dot-map", "agent-kit.json");
...
async function createBridgeServer({
  ...
  async function readLocalPayloadVersion() {
    try {
      const parsed = JSON.parse(await (0, import_promises7.readFile)((0, import_node_path9.join)(process.cwd(), "payload-manifest.json"), "utf8"));
  ...
  async function applyUpdate() {
    ...
    const updater = (0, import_node_path9.resolve)((0, import_node_path9.join)(process.cwd(), "..", "LiveDotMapSetup.exe"));
    try {
      await (0, import_promises7.access)(updater);
    } catch {
    ...
    const tempRoot = (0, import_node_path9.join)(process.env.TEMP || process.env.TMP || (0, import_node_os4.homedir)(), `livedot-update-${manifest.version}-${(0, import_node_crypto6.randomUUID)()}`);
    const payloadDir = (0, import_node_path9.join)(tempRoot, "payload");
    await (0, import_promises7.mkdir)(payloadDir, { recursive: true });
    try {
      for (const [relative3, meta] of Object.entries(manifest.files)) {
        ...
        const target = (0, import_node_path9.join)(payloadDir, relative3);
        await (0, import_promises7.mkdir)((0, import_node_path9.dirname)(target), { recursive: true });
        ...
        const actual = (0, import_node_crypto6.createHash)("sha256").update(buffer).digest("hex");
        ...
        await (0, import_promises7.writeFile)(target, buffer);
      }
    } catch (error) {
      await (0, import_promises7.rm)(tempRoot, { recursive: true, force: true }).catch(() => void 0);
      throw error;
    }
    const child = (0, import_node_child_process4.spawn)(updater, ["--update", tempRoot], { detached: true, stdio: "ignore", windowsHide: true });
    child.unref();
  ...
  const server = (0, import_node_http.createServer)(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/agents") {
        ...
        let config = {};
        try {
          const parsed = JSON.parse(await (0, import_promises7.readFile)((0, import_node_path9.join)(root, ".live-dot-map", "agent-kit.json"), "utf8"));
...
async function markdownDocuments2(root, limit = 200) {
  ...
  const walk = async (directory, depth) => {
    if (depth > 5 || output.length >= limit) return;
    const entries = await (0, import_promises8.readdir)(directory, { withFileTypes: true }).catch(() => []);
    for (const entry of entries) {
      if (output.length >= limit || ignored.has(entry.name)) continue;
      const full = (0, import_node_path10.join)(directory, entry.name);
      if (entry.isDirectory()) await walk(full, depth + 1);
      else if (entry.isFile() && (0, import_node_path10.extname)(entry.name).toLowerCase() === ".md") {
        const metadata = await (0, import_promises8.stat)(full).catch(() => null);
        if (!metadata || metadata.size > 2e6) continue;
        const text = await (0, import_promises8.readFile)(full, "utf8").catch(() => "");
        if (text && text.length <= 2e6) output.push({ path: full.slice(root.length + 1).replace(/\\/g, "/"), text });
      }
    }
  };
...
async function recordAgentHealth2(root, actor, event, status, error) {
  const path = (0, import_node_path10.join)(root, ".live-dot-map", ".bridge", "agent-health.json");
  const prior = await (0, import_promises8.readFile)(path, "utf8").then((text) => JSON.parse(text)).catch(() => ({}));
  const records = prior.records && typeof prior.records === "object" && !Array.isArray(prior.records) ? prior.records : {};
  const value = error;
  ...
  const temporary = `${path}.${process.pid}.${(0, import_node_crypto7.randomUUID)()}.tmp`;
  try {
    await (0, import_promises8.writeFile)(temporary, `${JSON.stringify({ version: 1, updatedAt: (/* @__PURE__ */ new Date()).toISOString(), records }, null, 2)}
`, "utf8");
...
async function openStore(projectRoot) {
  ...
  let mapName;
  try {
    const parsed = JSON.parse(await (0, import_promises8.readFile)((0, import_node_path10.join)(mapDirectory(root, activeMap), "map.json"), "utf8"));
...
function envelope(projectId, revision, actor, sessionId, commands) {
  return { projectId, baseRevision: revision, commandId: `cmd-${(0, import_node_crypto7.randomUUID)()}`, actor, sessionId, commands };
}
...
async function callTool(store, root, tool, args, defaultActor = "agent:generic") {
  ...
  const sessionId = `session-${(0, import_node_crypto7.randomUUID)()}`;
  if (tool === "map_get_context") {
  ...
  if (tool === "map_apply_commands") {
    const request = {
      ...
      baseRevision: Number.isInteger(args.baseRevision) ? args.baseRevision : snapshot.revision,
      commandId: typeof args.commandId === "string" ? args.commandId : `cmd-${(0, import_node_crypto7.randomUUID)()}`
    };
...
async function runHook(kind, args) {
  const root = (0, import_node_path10.resolve)(required(args, "project"));
  const actor = `agent:${String(args.agent || "generic")}`;
  const sessionId = String(args.session || `session-${(0, import_node_crypto7.randomUUID)()}`);
  const logger = createLogger({ source: "agent" });
  ...
  if (kind === "session-start") {
    const watermarkPath = (0, import_node_path10.join)(root, ".live-dot-map", "agent-read.json");
    let watermark = 0;
    try {
      const parsed = JSON.parse(await (0, import_promises8.readFile)(watermarkPath, "utf8"));
    ...
    if (changes.length || deliveredIds.length) {
      await (0, import_promises8.mkdir)((0, import_node_path10.dirname)(watermarkPath), { recursive: true });
      await (0, import_promises8.writeFile)(watermarkPath, `${JSON.stringify({ version: 1, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)}
`, "utf8");
...
async function main() {
  ...
  if (command2 === "serve") {
    ...
    const projectRoot = (0, import_node_path10.resolve)(required(args, "project"));
    const appPath = (0, import_node_path10.resolve)(typeof args.app === "string" ? args.app : (0, import_node_path10.join)(process.cwd(), "app.html"));
    const appHtml = await (0, import_promises8.readFile)(appPath, "utf8");
    const assetRoot = (0, import_node_path10.dirname)(appPath);
    const staticAssets = {};
    for (const [urlPath, file, type] of [
      ["/sw.js", "sw.js", "text/javascript; charset=utf-8"],
      ["/manifest.webmanifest", "manifest.webmanifest", "application/manifest+json; charset=utf-8"],
      ["/icons/icon-192.png", (0, import_node_path10.join)("icons", "icon-192.png"), "image/png"],
      ["/icons/icon-512.png", (0, import_node_path10.join)("icons", "icon-512.png"), "image/png"]
    ]) {
      try {
        staticAssets[urlPath] = { body: await (0, import_promises8.readFile)((0, import_node_path10.join)(assetRoot, file)), type };
      } catch {
      }
    }
...
`);
  ...
  if (command2 === "mcp") {
    const project = (0, import_node_path10.resolve)(typeof args.project === "string" && args.project.trim() ? args.project : process.cwd());
  ...
  if (command2 === "install") {
    const root = (0, import_node_path10.resolve)(typeof args.project === "string" ? args.project : process.cwd());
    const runtimeSource = process.env.LIVEDOT_RUNTIME_SOURCE || process.argv[1] || process.cwd();
    const appPath = (0, import_node_path10.resolve)(typeof args.app === "string" ? args.app : (0, import_node_path10.join)((0, import_node_path10.dirname)(runtimeSource), "app.html"));
...
void main().catch(async (error) => {
  ...
  if (parsed.command === "hook" || parsed.command === "mcp") {
    const project = (0, import_node_path10.resolve)(typeof parsed.args.project === "string" && parsed.args.project.trim() ? parsed.args.project : process.cwd());
