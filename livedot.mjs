...
async function withFileLock(path, operation, { timeoutMs = 5e3, staleMs = 3e4 } = {}) {
  ...
  try {
    ...
    await unlink(path).catch((error3) => {
...
// src/bridge/maps.mjs
import { copyFile as copyFile2, cp, lstat, mkdir as mkdir2, readdir, readFile as readFile2, rename as rename2, rm } from "node:fs/promises";
...
async function listMapIds(projectRoot) {
  const entries = await readdir(mapsRoot(projectRoot), { withFileTypes: true }).catch((error3) => {
...
async function createMap(projectRoot, name, { now = () => /* @__PURE__ */ new Date() } = {}) {
  ...
  for (let suffix = 1; suffix < 1e5; suffix += 1) {
    ...
    try {
      await mkdir2(mapDirectory(projectRoot, id));
...
async function ensureBundleLayout(projectRoot, mapId) {
  ...
  const mapPath = join2(root, "map.json");
  if (!await exists(mapPath)) return { migrated: false, mapId };
  const lockPath = join2(root, ".bridge", "write.lock");
  try {
...
async function ensureBundleLayoutLocked(projectRoot, mapId, root, mapPath) {
  ...
  for (const kind of ["nodes", "routes"]) {
    const directory = join2(root, kind);
    await rejectSymlink(directory);
    const entries = await readdir(directory, { withFileTypes: true }).catch((error3) => error3?.code === "ENOENT" ? [] : (() => {
      throw error3;
    })());
    for (const entry of entries) {
      ...
      const source = join2(directory, entry.name);
      const destination = join2(directory, ownerId, "index.md");
  ...
  const backupDirectory = join2(root, ".bridge", "backups", `pre-bundle-migration-${stamp}`);
  const journalPath = join2(root, ".bridge", "migrations", "bundle-layout-v1.json");
  await ensureDirectory(backupDirectory);
  await ensureDirectory(dirname2(journalPath));
  await copyFile2(mapPath, join2(backupDirectory, "map.json"));
  for (const item of planned) {
    const backup = join2(backupDirectory, item.relative);
    await ensureDirectory(dirname2(backup));
    await copyFile2(item.source, backup);
  }
  await writeJsonAtomic(journalPath, { version: 1, state: "prepared", mapId, planned: planned.map((item) => item.relative), completed: [] });
  const completed = [];
  try {
    for (const item of planned) {
      await ensureDirectory(dirname2(item.destination));
      await rename2(item.source, item.destination);
      completed.push(item);
    ...
    for (const item of completed.reverse()) {
      await ensureDirectory(dirname2(item.source));
      await rename2(item.destination, item.source).catch(() => void 0);
    }
...
async function rejectSymlink(path) {
  const metadata = await lstat(path).catch((error3) => {
    if (error3?.code === "ENOENT") return null;
    throw error3;
  });
...
function within(root, candidate) {
  const value = relative3(root, candidate);
  return value === "" || value !== ".." && !value.startsWith(`..${sep2}`) && !/^[A-Za-z]:[\\/]/.test(value);
}
...
function titleMarkdown(name, title) {
  const fallback = basename2(name, extname(name)).slice(0, 80) || "\u672A\u547D\u540D\u8D44\u6599";
...
async function safeLstat(path) {
  return lstat4(path).catch((error3) => {
    if (error3?.code === "ENOENT") return null;
    throw error3;
  });
}
var BundleStore = class _BundleStore {
  ...
  async #prepareOwner(info) {
    ...
    await mkdir5(info.directory, { recursive: true });
    await this.#assertSafePath(this.projectRoot, info.directory, { allowMissing: false });
  }
  async #entries(info, { includeArchived = false } = {}) {
    await this.#assertSafePath(this.projectRoot, info.directory, { allowMissing: true });
    const active = await readdir4(info.directory, { withFileTypes: true }).catch((error3) => {
      if (error3?.code === "ENOENT") return [];
      throw error3;
    });
    const output = [];
...
${right}`;
  ...
  async #copySource(sourcePath, temporary, { allowExternal = false } = {}) {
    ...
    try {
      ...
      handle = await open2(candidate, flags);
...
var MapManager = class _MapManager {
  ...
  async rename(mapKey, name, actor = "human") {
    ...
    return context.store.execute({
      ...
      commandId: `map-rename-${randomUUID2()}`,
      actor,
      sessionId: `map-manager-${randomUUID2()}`,
...
function within2(root, candidate) {
  ...
  return value === "" || value !== ".." && !value.startsWith(`..${sep3}`) && !/^[A-Za-z]:[\\/]/.test(value);
}
...
async function requireRegularPath(root, candidate, { allowMissing = false } = {}) {
  const rootReal = await realpath5(root).catch((error3) => {
...
async function collect({ projectRoot, mapKey, document, includeHistory = false, mdIndex = null } = {}) {
  ...
  const mapMetadata = await lstat5(mapRoot).catch((error3) => error3?.code === "ENOENT" ? null : (() => {
    throw error3;
  })());
  ...
  const addAsset = async (candidate, owner2, name) => {
    ...
    const type = ASSET_TYPES2[extname2(name).toLowerCase()] ?? { kind: "file", mimeType: "application/octet-stream" };
    const info = {
      path: relativePath,
      fileName: name,
      name,
...
var ToolService = class {
  ...
  async #context(args = {}) {
    ...
    return {
      ...
      commandId: typeof args.commandId === "string" ? args.commandId : `${prefix}-${randomUUID3()}`,
      actor: this.actor,
      sessionId: typeof args.sessionId === "string" ? args.sessionId : `tool-${randomUUID3()}`,
      commands
    };
  }
...
async function assertPackagePath(path) {
  const metadata = await lstat6(path).catch((cause) => {
      ...
      try {
        ...
        try {
          const committed = await purgeStore.execute({
            ...
            commandId: String(options.commandId ?? `purge-${randomUUID4()}`),
            actor: "system:purge",
        ...
        await rm4(txnRoot, { recursive: true, force: true }).catch(() => void 0);
...
function defaultNativeHelperPath(options = {}) {
  if (options.helperPath) return resolve10(options.helperPath);
...
async function assertPurgeStagingPath(value) {
  ...
  const leaf = await lstat7(target);
...
async function readAppPaths(exeNames, execFile3 = childExecFile) {
...
async function readVSCodeAppPaths(execFile3 = childExecFile) {
...
var EditorService = class _EditorService {
  ...
  async #assertProjectRoot() {
    let metadata;
    try {
      metadata = await lstat8(this.projectRoot);
    } catch (error3) {
  ...
  async #assertNoSymlinkEscape(candidate, { allowMissing = false, root = this.projectRoot } = {}) {
    ...
    const canonicalCandidate = await realpath6(current);
...
var NativeWindowsHelper = class {
  ...
  async run(request = {}) {
    ...
    await access2(this.helperPath).catch(() => {
...
async function sha256File(path) {
  try {
    return createHash8("sha256").update(await readFile11(path)).digest("hex");
  } catch {
    return null;
  }
}
async function refreshAgentRuntime({ runtimeSource, homeRoot } = {}) {
  ...
  const source = resolve15(runtimeSource);
  const target = join18(resolve15(homeRoot || homedir6()), ".live-dot-map", "livedot.mjs");
...
async function createBridgeServer({
  ...
  async function readLocalPayloadInfo() {
    try {
      const parsed = JSON.parse(await readFile11(join18(installRoot, "payload-manifest.json"), "utf8"));
      return {
  ...
  async function applyUpdate() {
    ...
    try {
      for (const [relative6, meta] of Object.entries(manifest.files)) {
        ...
        await downloadUpdateFile(meta, join18(payloadDir, relative6), relative6);
      }
    } catch (error3) {
      await rm6(tempRoot, { recursive: true, force: true }).catch(() => void 0);
      throw error3;
    }
    let updater;
    if (manifest.installer && typeof manifest.installer === "object") {
      updater = join18(tempRoot, "LiveDotMapSetup.exe");
      try {
      ...
      updater = resolve15(join18(installRoot, "..", "LiveDotMapSetup.exe"));
      try {
    ...
    const updaterArgs = ["--update", join18(tempRoot, "LiveDotMapSetup.exe"), resolve15(join18(installRoot, ".."))];
    if (spawnUpdater) {
      spawnUpdater(updater, updaterArgs);
    } else {
      const child = spawn3(updater, updaterArgs, { detached: true, stdio: "ignore", windowsHide: true, cwd: tempRoot });
      child.unref();
    }
    ...
    try {
      ...
      if (pathname === "/archive/restore") {
        ...
        const result2 = await store.execute({
          ...
          commandId: `human-restore-${randomUUID8()}`,
          actor: "human",
      ...
      if (pathname === "/archive/purge") {
        ...
        const result2 = await lifecycle.purge({
          ...
          actor: "human",
          confirmed: true,
          commandId: `human-purge-${randomUUID8()}`
        });
        sendJson(response, 200, result2);
        return;
      }
      ...
      try {
        ...
        for (const map of listed.maps) {
          ...
          for (const item of await lifecycle.listEligible({ now: clock() })) {
            await lifecycle.purge({ ...item, actor: "system:retention", now: clock(), commandId: `retention-purge-${randomUUID8()}` });
          }
        }
      } catch (error3) {
...
`, "utf8");
  ...
  return async () => {
    ...
    released = true;
    await handle.close().catch(() => void 0);
    await rm7(path, { force: true }).catch(() => void 0);
  };
}
...
async function removeBridgeState(runtimeStateDir, expectedPid) {
  ...
  await rm7(paths.bridge, { force: true });
  return true;
}
...
async function inspectProjectQualification(projectRoot) {
  ...
  const writableTarget = packageMap ? await access5(mapsPath, constants3.W_OK).then(() => true).catch(() => false) : await access5(dataDirectory, constants3.W_OK).then(() => true).catch(() => false);
...
async function resolveAppHtmlPath(explicit) {
  ...
  for (const candidate of [...new Set(candidates)]) {
    if (await access5(candidate, constants3.F_OK).then(() => true).catch(() => false)) return candidate;
  }
  return null;
}
async function igniteBridge(projectRoot, appPath, runtimeStateDir) {
  const script = process.argv[1] ? resolve17(process.argv[1]) : "";
  const serveArgs = [
    ...isSea() || !script ? [] : [script],
    "serve",
    "--project",
  ...
  const child = spawn4(process.execPath, serveArgs, { detached: true, stdio: "ignore", windowsHide: true });
  child.unref();
}
...
async function main() {
  ...
  if (command2 === "serve") {
    ...
    if (state) {
      ...
      if (image && looksLikeBridge && resolve17(image).toLowerCase() !== resolve17(process.execPath).toLowerCase()) {
...
`);
  ...
  if (command2 === "mcp") {
    ...
    return runMcp(project, `agent:${String(args.agent || "generic")}`, {
      runtimeStateDir: typeof args["runtime-state-dir"] === "string" ? resolve17(args["runtime-state-dir"]) : void 0,
      appPath: typeof args.app === "string" ? resolve17(args.app) : void 0
    });
  }
  if (command2 === "hook") {
    const project = resolve17(typeof args.project === "string" && args.project.trim() ? args.project : process.cwd());
